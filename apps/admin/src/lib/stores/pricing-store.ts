...
/**
 * Pricing/Membership store — dynamic plans editable from /admin/pricing.
...
const read = (): PricingContent => {
  const v = driver.readJson<PricingContent | null>(RES, null as any);
  if (!v) return DEFAULTS;
  return { ...DEFAULTS, ...v, plans: v.plans ?? DEFAULTS.plans, faqs: v.faqs ?? DEFAULTS.faqs };
};

export const pricingStore = {
  get: read,
  save: (c: PricingContent) => {
    void api.put("/pricing", c).catch(() => {});
  },
  reset: () => {
    void api.del("/pricing").catch(() => {});
  },
};

export function usePricingContent() {
  const [content, setContent] = useState<PricingContent>(() => read());
  useEffect(() => {
    const refresh = () => setContent(read());
    // Kick a fetch so the mock (or future real API) is the source of truth.
    void api.get<PricingContent>("/pricing").catch(() => {});
