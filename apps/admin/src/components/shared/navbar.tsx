...
import { usePublicContent } from "@/lib/stores/public-content-store";
...
const SharedNavbar = ({ backTo, backLabel, rightContent, showAuth }: SharedNavbarProps) => {
  ...
  const { categories: allCourseCategories } = usePublicContent();
  const courseCategories = allCourseCategories.filter((c) => !c.parentId);
