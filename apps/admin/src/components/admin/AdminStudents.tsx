...
const AdminStudents = ({ initialStudents }: { initialStudents: StudentRow[] }) => {
  ...
  const totalEnrollments = initialStudents.reduce((sum, s) => sum + s.enrolledCourses.length, 0);
  const completedCount = initialStudents.reduce((sum, s) => sum + s.completedCourses, 0);

  return (
    <div className="space-y-4 sm:space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold tracking-tight">শিক্ষার্থী</h1>
        <p className="text-muted-foreground text-xs sm:text-sm mt-1">{initialStudents.length} জন শিক্ষার্থী</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          { label: "মোট শিক্ষার্থী", value: initialStudents.length, icon: UserCheck, color: "text-primary" },
                      ...
                      {student.enrolledCourses.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {student.enrolledCourses.slice(0, 3).map((c) => (
                            <span key={c.id} className="text-[10px] px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground">
                              {c.title}
                            </span>
                          ))}
