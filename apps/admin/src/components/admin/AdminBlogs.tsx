...
import { generateSitemapXml } from "@/server/actions/sitemap.actions";
import { useRouter } from "next/navigation";
import { type ManagedBlogPost, type BlogStatus } from "@/types/blog";
import { updateBlog, deleteBlog } from "@/server/actions/blog.actions";
...
/**
 * Admin Blogs — composition shell.
 */
const AdminBlogs = ({ initialPosts, initialCategories, authors }: { initialPosts: ManagedBlogPost[], initialCategories: BlogCategory[], authors: Author[] }) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex gap-2">
          <Button
            ...
            onClick={async () => {
              ...
              toast({ title: "sitemap.xml ডাউনলোড হয়েছে", description: "public/sitemap.xml এ replace করে deploy করুন।" });
            }}
          >
            <Download className="h-4 w-4" /> Sitemap এক্সপোর্ট
          </Button>
      ...
      <BlogBulkBar
        count={selected.size}
        onPublish={() => runBulk((id) => updateBlog(id, { published: true } as any), "{n} টি প্রকাশিত")}
        onUnpublish={() => runBulk((id) => updateBlog(id, { published: false } as any), "{n} টি ড্রাফট-এ")}
        onFeature={() => runBulk((id) => updateBlog(id, { featured: true } as any), "ফিচার্ড হিসেবে চিহ্নিত")}
