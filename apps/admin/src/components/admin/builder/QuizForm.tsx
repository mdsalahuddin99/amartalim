...
interface Props {
  initial?: any;
  quizQuestions: any[];
  onSaveMeta: (v: any) => Promise<string>;
  onAddQuestion: (quizId: string, q: any) => Promise<void> | void;
  onUpdateQuestion: (id: string, q: any) => Promise<void> | void;
...
type QuizQuestion = {
  ...
  type?: "mcq" | "true-false" | "fill" | "short";
};
...
export const QuizForm = ({
  ...
  const saveMeta = async (showToast = true) => {
    if (!title.trim()) {
      if (showToast) toast.error("শিরোনাম আবশ্যক");
      return null;
    }
    const id = await onSaveMeta({
      title: title.trim(),
  ...
  const saveQuestion = async () => {
    ...
    if (!id) {
      id = (await saveMeta(false)) || undefined;
      if (!id) { toast.error("আগে কুইজের শিরোনাম দিন"); return; }
    }
    if (!qForm.question.trim()) return toast.error("প্রশ্ন আবশ্যক");
    if (qForm.type === "mcq" && qForm.options.some((o: string) => !o.trim())) return toast.error("সকল অপশন পূরণ করুন");
    if (editingQId) {
      await onUpdateQuestion(editingQId, qForm);
      toast.success("প্রশ্ন আপডেট হয়েছে");
    } else {
      await onAddQuestion(id, qForm);
    ...
    <>
      ...
      <div className="space-y-4 py-4">
        <div className="rounded-xl border p-3 space-y-3 bg-secondary/20">
          ...
          <Button size="sm" variant="outline" className="rounded-xl w-full text-xs h-8" onClick={async () => { await saveMeta(); toast.success("সেটিংস সংরক্ষিত"); }}>
