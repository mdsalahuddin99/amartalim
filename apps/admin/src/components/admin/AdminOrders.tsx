...
import { Receipt, Search, RotateCcw, CheckCircle2, XCircle, Clock, Eye, Loader2 } from "lucide-react";
...
const AdminOrders = ({ initialOrders }: { initialOrders: any[] }) => {
  ...
  const refund = (o: any) => {
    ...
    updateStatus(o.id, "REFUNDED");
  };
  const markPaid = (o: any) => updateStatus(o.id, "PAID");
  const markFailed = (o: any) => updateStatus(o.id, "FAILED");
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t">
                {selected.status !== "PAID" && (
                  <Button size="sm" onClick={() => markPaid(selected)} disabled={isPending} className="gap-1">
                    {isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle2 className="h-3.5 w-3.5" />} পেইড মার্ক করুন
                  </Button>
                )}
                {selected.status === "PENDING" && (
                  <Button size="sm" variant="outline" onClick={() => markFailed(selected)} disabled={isPending} className="gap-1">
                    {isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <XCircle className="h-3.5 w-3.5" />} ব্যর্থ মার্ক করুন
                  </Button>
                )}
                {selected.status === "PAID" && (
                  <Button size="sm" variant="outline" className="text-destructive gap-1" onClick={() => refund(selected)} disabled={isPending}>
                    {isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RotateCcw className="h-3.5 w-3.5" />} রিফান্ড
                  </Button>
                )}
              </div>
            </div>
