...
import { createCourse, updateCourse, deleteCourse, createLesson, updateLesson, deleteLesson } from "@/server/actions/admin-courses";

type Course = {
  id: string; title: string; description?: string; categoryId: string; categoryName: string; price: number; duration: string;
  level: "শিক্ষানবিস" | "মধ্যবর্তী" | "উন্নত"; instructor: string; instructorBio?: string; thumbnail?: string;
  whatYouLearn?: string[]; whoIsFor?: string[]; benefits?: string[]; problems?: any[];
  lessonsCount: number; studentsCount: number; rating: number;
};
type Lesson = {
  id: string; courseId: string; title: string; description?: string; youtubeId?: string; duration?: string; order: number;
};
...
const AdminCourses = ({ initialCourses, initialCategories }: { initialCourses: any[], initialCategories: any[] }) => {
  const navigate = useNavigate();
  const [courses, setCourses] = useState<any[]>(initialCourses);
  const [lessons, setLessons] = useState<any[]>(initialCourses.flatMap(c => c.lessons || []));
  ...
  const handleCreateCourse = () => {
    // not used from modal currently in the provided code, user clicks button and goes to builder
  };
  ...
  const openEditCourse = (course: Course) => {
    ...
    setCourseForm({
      ...
      price: course.price, duration: course.duration, level: course.level, instructor: course.instructor,
  ...
  const handleUpdateCourse = () => {
    ...
    setIsPending(true);
    updateCourse(editingCourseId, courseForm).then(() => {
  ...
  const handleCreateLesson = () => {
    ...
    setIsPending(true);
    createLesson({ ...lessonForm, courseId: activeLessonCourseId, order: courseLessons.length + 1 }).then(() => {
      // optimistic update is a bit tricky without ID, but we just reload for now
  ...
  const openEditLesson = (lesson: Lesson) => {
    ...
    setLessonForm({ title: lesson.title, description: lesson.description, youtubeId: lesson.youtubeId, duration: lesson.duration });
  ...
  const handleUpdateLesson = () => {
    ...
    setIsPending(true);
    updateLesson(editingLessonId, lessonForm).then(() => {
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Button
          ...
          onClick={() => {
            const defaultCat = initialCategories[0];
            setIsPending(true);
            createCourse({
              title: "নতুন কোর্স",
              description: "কোর্সের বর্ণনা...",
          ...
          disabled={isPending}
        >
          {isPending ? "অপেক্ষা করুন..." : <><Plus className="mr-2 h-4 w-4" /> কোর্স যোগ করুন</>}
        </Button>
      </div>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-2xl">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <DialogClose asChild><Button variant="outline" className="rounded-xl w-full sm:w-auto" disabled={isPending}>বাতিল</Button></DialogClose>
            <Button onClick={handleUpdateCourse} className="rounded-xl w-full sm:w-auto" disabled={isPending}>সংরক্ষণ করুন</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={lessonDialogOpen} onOpenChange={setLessonDialogOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-lg">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <DialogClose asChild><Button variant="outline" className="rounded-xl w-full sm:w-auto" disabled={isPending}>বাতিল</Button></DialogClose>
            <Button onClick={handleCreateLesson} className="rounded-xl w-full sm:w-auto" disabled={isPending}>পাঠ যোগ করুন</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={editLessonDialogOpen} onOpenChange={setEditLessonDialogOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-lg">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <DialogClose asChild><Button variant="outline" className="rounded-xl w-full sm:w-auto" disabled={isPending}>বাতিল</Button></DialogClose>
            <Button onClick={handleUpdateLesson} className="rounded-xl w-full sm:w-auto" disabled={isPending}>সংরক্ষণ করুন</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
