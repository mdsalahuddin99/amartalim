...
import { createUserBlog as createBlog, updateUserBlog as updateBlog } from "@/server/actions/user-blog.actions";
...
export function useBlogDraft(blogCategories: BlogCategory[]): UseBlogDraft {
  ...
  const saveDraft = async () => {
    ...
    await runAction(async () => {
      ...
      if (editing) {
        const res = await updateBlog(editing.id, { ...payload, status: "draft", published: false, publishAt: "" } as any);
        if (!res.ok) throw new Error(res.error || "Failed to update draft");
        toast({ title: "ড্রাফট সংরক্ষিত হয়েছে" });
      } else {
        const res = await createBlog({ ...payload, status: "draft", published: false } as any);
  ...
  const submitForReview = async () => {
    ...
    await runAction(async () => {
      const payload = buildPayload(validated as any);
      if (editing) {
        const res = await updateBlog(editing.id, { ...payload, status: "pending", published: false, publishAt: "" } as any);
        if (!res.ok) throw new Error(res.error || "Failed to submit for review");
        toast({ title: "রিভিউয়ের জন্য সাবমিট হয়েছে" });
      } else {
        const res = await createBlog({ ...payload, status: "pending", published: false } as any);
        if (!res.ok) throw new Error(res.error || "Failed to submit blog");
        toast({ title: "রিভিউয়ের জন্য সাবমিট হয়েছে" });
      }
      close();
  ...
  return {
    ...
    saveDraft, publishNow: submitForReview, schedulePost: submitForReview,
  };
}
