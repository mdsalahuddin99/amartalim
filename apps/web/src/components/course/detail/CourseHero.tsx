    ...
    <div className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-20 sm:pt-28 pb-8 sm:pb-12">
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-5 mt-5 text-xs sm:text-sm text-background/60">
          ...
          <span className="flex items-center gap-1.5"><Users className="h-4 w-4" />{course.studentCount || 0}+ শিক্ষার্থী</span>
          {course.duration && <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" />{course.duration}</span>}
          <span className="flex items-center gap-1.5"><BookOpen className="h-4 w-4" />{course.lessons?.length || 0}টি পাঠ</span>
        ...
        {course.instructor?.name && (
          <div className="flex items-center gap-3 mt-5">
            ...
            <div>
              <p className="text-sm font-semibold text-background">{course.instructor.name}</p>
