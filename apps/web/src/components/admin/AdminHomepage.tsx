...
import { updateHomepageContent } from "@/server/actions/homepage.actions";

const uid = () => `${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;

const AdminHomepage = () => {
  const [draft, setDraft] = useState<HomepageContent>(HOMEPAGE_DEFAULTS);
  ...
  const save = async () => {
    const res = await updateHomepageContent(draft);
    if (res.ok) {
  ...
  const reset = async () => {
    ...
    await updateHomepageContent(HOMEPAGE_DEFAULTS);
