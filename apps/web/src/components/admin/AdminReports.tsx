...
import { getAdminReportsData } from "@/server/actions/admin-dashboard.actions";
...
const AdminReports = () => {
  ...
  useEffect(() => {
    getAdminReportsData().then((res) => {
      setData({ ...res, activeStudentsCount: res.totalEnrollments });
    });
  }, []);
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          ...
          { label: "সক্রিয় শিক্ষার্থী", value: data.activeStudentsCount, icon: Users, trend: "+৮%", up: true, color: "text-primary", bg: "bg-primary/10" },
          { label: "গড় রেটিং", value: data.avgRating, icon: Star, trend: "Top", up: true, color: "text-accent", bg: "bg-accent/10" },
        ].map((s) => (
        ...
        <div className="lg:col-span-2 rounded-2xl bg-card border border-border/50 shadow-card p-4 sm:p-5">
          ...
          <div className="space-y-3">
            {data.topCourses.map((c, i) => {
              const max = data.topCourses[0].revenue;
              return (
                <div key={c.name}>
