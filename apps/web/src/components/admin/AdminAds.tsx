...
const AdminAds = () => {
  ...
  const [cfg, setCfg] = useState({ adsenseEnabled: false, adsenseClientId: "" });
  ...
  useEffect(() => {
    getAdsData().then((res) => {
      ...
      setCfg({
        adsenseEnabled: res.config?.adsenseEnabled || false,
        adsenseClientId: res.config?.adsenseClientId || "",
      });
    });
  }, []);
  ...
  const save = async () => {
    ...
    let newItems = [...items];
    if (editing) {
      newItems = newItems.map((a) => (a.id === editing.id ? { ...a, ...form } : a));
      toast.success("আপডেট হয়েছে");
    } else {
      newItems.push({ ...form, id: `ad_${Date.now()}`, createdAt: new Date().toISOString() });
      toast.success("বিজ্ঞাপন যোগ হয়েছে");
    }
    
    setItems(newItems);
    await updateAdsData({ items: newItems, config });
    setOpen(false);
  };

  const saveCfg = async () => {
    setConfig(cfg);
    await updateAdsData({ items, config: cfg });
  ...
  const toggleAd = async (id: string) => {
    const newItems = items.map(a => a.id === id ? { ...a, enabled: !a.enabled } : a);
    setItems(newItems);
    await updateAdsData({ items: newItems, config });
  };
              ...
              {items.map((a) => (
                <div key={a.id} className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => toggleAd(a.id)} title={a.enabled ? "বন্ধ করুন" : "চালু করুন"}>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={() => removeAd(a.id)}>মুছুন</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  </div>
